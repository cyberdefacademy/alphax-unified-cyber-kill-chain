...
**Live-fire notes (2026-09-05, engagement `1c1e71b8`, SQLi WHERE-clause labs):** nmap connect fingerprint `443/tcp open https` (AWS eu-west-1, ~1–4s); nuclei `info,low` found `wildcard-dns-detect` only; nikto `exit=0` with `[FAIL] Unable to connect` after expiry (nikto's own exit-code quirk — stdout tells the truth); graph correctly ingested `34.246.129.62` + instance hostname. Second instance (`0a3a00…00ee`): edge began **filtering SYN connects** (`443/tcp filtered`) while HTTPS still served — service-layer tools (nuclei/nikto/curl) unaffected since they use normal TLS. **P5 sqlmap** (`filter?category=Gifts`, level 1, 300s cap): confirmed `category` is **AND boolean-based blind (WHERE/HAVING) injectable, PostgreSQL backend** before the timeout tripped (`exit=124`, partial output preserved, `blocked_needs_input` + pivot fired as designed); manual 3-request boolean probe then confirmed TRUE/FALSE differential (7833 vs 6641 bytes) and `OR 1=1--` retrieved hidden products (14.5KB vs 7.8KB baseline) — lab objective met with 4 total requests. Bugs fixed during these runs: immutable engagement scope (`schemas.py:23` + router now accept `name`/`scope_cidr`), nmap text parser hostname loss + version line-bleed + multi-host mis-attachment (`parsers/nmap_parser.py:43` — `[ \t]+` separators, hostname map, positional sweep), stale graph rows on shared IPs (executor now **upserts** hostname/ports instead of skipping duplicates).
